...
void AddCofaces(const std::vector<std::vector<int>>& g, int depth, int max_depth,
                std::vector<int> cur_node, std::vector<int> neighbors, SimplicialComplex* simpl) {
    if (depth > max_depth) {
        return;
    }
...
std::unique_ptr<SimplicialComplex> SimplicialComplex::CreateCliqueGraph(
    const std::vector<std::vector<int>>& g, int k, int method, int total_threads) {
    int n = g.size();
    if (n == 0) {
    ...
    if (method == 0) {  // incremental
        ...
        auto AddSubsetVertices = [&](int l, int r, SimplicialComplex* thread_result) {
            for (int i = l; i < r; i++) {
                int v = i;
                std::vector<int> N;
                for (int u = 0; u < v; u++) {
...
void SimplicialComplex::BuildFromBinary(std::vector<std::vector<int>> binary, bool on_column) {
