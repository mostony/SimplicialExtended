...
TEST_CASE("Simplicial Complex incidence") {
    ...
    // v = 1, k = 1
    {
        ...
        REQUIRE_THAT(incidence, Catch::Matchers::UnorderedEquals(std::vector<std::vector<int>>(
                                    {{1, 4}, {1, 3}, {1, 2}, {1, 6}, {1, 5}})));
        auto incidence_degree = simpl.Degree({1}, 1);
    ...
    // v = 1, k = 2
    {
        ...
        auto incidence_degree = simpl.Degree({1}, 2);
    ...
    // v = 1, k = 3
    {
        ...
        auto incidence_degree = simpl.Degree({1}, 3);
...
TEST_CASE("Combinatorial Complex incidence") {
    ...
    comb.Build({{1}, {2}, {3}, {4}, {1, 2}, {1, 4}, {2, 4}, {1, 2, 3}, {2, 3, 4}});
    ...
    // v = 1, k = 1
    {
        ...
        auto incidence_degree = comb.Degree({1}, 1);
    ...
    // v = 1, k = 2
    {
        ...
        REQUIRE_THAT(incidence,
                     Catch::Matchers::UnorderedEquals(std::vector<std::vector<int>>({{1, 2, 3}})));
        auto incidence_degree = comb.Degree({1}, 2);
        REQUIRE(incidence_degree == 1);
    }
}
...
TEST_CASE("Combinatorial Complex degree") {
    ...
    // v = 1, k = 1
    {
        ...
        REQUIRE_THAT(adjacency,
                     Catch::Matchers::UnorderedEquals(std::vector<std::vector<int>>({{2}, {4}})));
    ...
    // v = 1, k = 2
    {
        ...
        REQUIRE_THAT(adjacency,
                     Catch::Matchers::UnorderedEquals(std::vector<std::vector<int>>({{2}, {3}})));
...
TEST_CASE("Simplex fvector & total count") {
    ...
    REQUIRE_THAT(simpl.FVector(), Catch::Matchers::UnorderedEquals(std::vector<std::pair<int, int>>(
                                      {{0, 6}, {1, 9}, {2, 5}, {3, 1}})));
...
TEST_CASE("SubgraphCentrality") {
    SimplicialComplex sc;
    sc.AddSimplex({1, 2, 3, 4});
    sc.AddSimplex({3, 4, 5});
    sc.AddSimplex({4, 5, 6});
    sc.AddSimplex({6, 7, 8});
    sc.AddSimplex({6, 9});
    auto sub = sc.SubgraphCentrality(1, 2);
    std::vector<int> test_simpl = {1, 4};
    for (auto [simpl, val] : sub) {
        if (simpl == test_simpl) {
            REQUIRE(val == Catch::Approx(2.714).epsilon(0.01));
        }
    }
}

TEST_CASE("Graph common neighbors") {
    Graph g;
    g.AddEdge(0, 1);
    g.AddEdge(0, 2);
    REQUIRE(g.CommonNeighbors({0}, {1}, 1) == Catch::Approx(0));
    g.AddEdge(1, 2);
    REQUIRE(g.CommonNeighbors({0}, {1}, 1) == Catch::Approx(1));

    g.Clear();
    g.AddEdge(0, 1);
    g.AddEdge(2, 3);
    REQUIRE(g.CommonNeighbors({0}, {2}, 1) == Catch::Approx(0));
    g.AddEdge(1, 2);
    REQUIRE(g.CommonNeighbors({0}, {2}, 1) == Catch::Approx(1));
}

TEST_CASE("Simplicial complex common neighbors") {
    SimplicialComplex sc;
    sc.AddSimplex({0, 1, 2});
    sc.AddSimplex({2, 3});
    REQUIRE(sc.CommonNeighbors({0}, {3}, 1) == Catch::Approx(1));
    REQUIRE(sc.CommonNeighbors({0}, {3}, 2) == Catch::Approx(0));

    REQUIRE(sc.CommonNeighbors({0}, {2}, 1) == Catch::Approx(1));
    REQUIRE(sc.CommonNeighbors({0}, {2}, 2) == Catch::Approx(1));
}
